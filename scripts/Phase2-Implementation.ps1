...
# 1. 最新の指示書を抽出（逆順スキャン）
Write-Host "Issue #$IssueNumber から指示書を抽出中..."
...
if (-not $Instructions) {
    Write-Error "🚨 指示書（非表示タグ）が見つかりません。Refinementが正常に完了しているか確認してください。"
    exit 1
}

# --- 追加部分: Feature Nameの抽出とScaffoldingの実行 ---
Write-Host "Issueタイトルから機能名を抽出し、Scaffoldingを実行します..."
$IssueInfo = gh issue view $IssueNumber --json title | ConvertFrom-Json
$FeatureName = ($IssueInfo.title -split '[: 　]')[0]
Write-Host "対象機能名: $FeatureName"

# Scaffoldingスクリプトの呼び出し
...
if (Test-Path $TargetDir) {
    ...
    Write-Warning "Scaffoldingディレクトリが見つかりません。新規作成ではなく既存改修の可能性があります。"
}
# --------------------------------------------------------
...
# Aider実行後のVRAM解放
Invoke-RestMethod -Uri "http://localhost:11434/api/generate" -Method Post -Body '{"model":"qwen2.5-coder:7b","keep_alive":0}' -ContentType "application/json"
